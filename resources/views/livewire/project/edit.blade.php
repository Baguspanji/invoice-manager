...
new class extends Component {
    ...
    {
        $view->title('Edit Proyek');
    }

    // columns for project editing
    ...
    /**
     * Mount project data on component load
     */
    public function mount(Project $project): void
    {
        $this->project = $project->load('items');
        $this->project_number = $project->project_number;
        $this->client_id = $project->client_id;
        $this->name = $project->name;
        $this->start_date = Carbon::parse($project->start_date)->format('Y-m-d');
        $this->due_date = Carbon::parse($project->due_date)->format('Y-m-d');
        ...
        if ($project->items->count() > 0) {
            ...
            // Initialize with one empty project item if none exist
            $this->projectItems[] = [
                'name' => '',
                'quantity' => 1,
                'unit_price' => 0.0,
    ...
    public function submit(): void
    {
        ...
        DB::transaction(function () {
            ...
            foreach ($this->projectItems as $item) {
                ...
                $this->project->items()->create($item);
            }
        });
